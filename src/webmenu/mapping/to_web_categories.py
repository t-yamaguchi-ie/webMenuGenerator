...
def _get_image_path(
    ...
    if has_osusume:
        ...
        if os.path.isfile(file_path):
            result["default"] = f"osusume_images/images/{name}"
    ...
    for lang in multi_lang_dirs:
        ...
        if has_osusume:
            ...
            if os.path.isfile(file_path):
                result[lang] = f"osusume_images/images/{lang}/{name}"
                continue
...
def make_categories(free_dir:str, osusume_dir:str, menudb, ini_bundle, small_pages, schema_version="0.1"):
    ...
    for lmenu in lmenus:
        ...
        # 大分類関連画像を取得する
        l_scr_btn_img = _get_image_path(free_dir, osusume_dir, lmenu.get("scrBtnImg", ""), True, multi_lang_dirs)
        l_frm_back_img = _get_image_path(free_dir, osusume_dir, lmenu.get("lmFrmBackImg", ""), False, multi_lang_dirs)
        l_frm_off_btn_img = _get_image_path(free_dir, osusume_dir, lmenu.get("frmOffBtnImg", ""), True, multi_lang_dirs)
        l_frm_on_btn_img = _get_image_path(free_dir, osusume_dir, lmenu.get("frmOnBtnImg", ""), True, multi_lang_dirs)
        l_m_frm_back_img = _get_image_path(free_dir, osusume_dir, lmenu.get("mmFrmBackImg", ""), False, multi_lang_dirs)
        children = []
        for mmenu in mmenus:
            ...
            # 中分類関連画像を取得する
            m_btn_on_img = _get_image_path(free_dir, osusume_dir, mmenu.get("btnOnImg", ""), True, multi_lang_dirs)
            m_btn_off_img = _get_image_path(free_dir, osusume_dir, mmenu.get("btnOffImg", ""), False, multi_lang_dirs)
